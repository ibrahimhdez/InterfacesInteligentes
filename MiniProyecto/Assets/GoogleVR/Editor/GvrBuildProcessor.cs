...
class GvrBuildProcessor : IPreprocessBuild {
  ...
  private bool IsDaydreamSDKIncluded() {
    return UnityEngine.VR.VRSettings.supportedDevices.Contains(GvrSettings.VR_SDK_DAYDREAM);
  }

  // 'Player Settings > Virtual Reality SDKs' includes 'Cardboard'?
  private bool IsCardboardSDKIncluded() {
    return UnityEngine.VR.VRSettings.supportedDevices.Contains(GvrSettings.VR_SDK_CARDBOARD);
  }
}
